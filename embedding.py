...
class BertEmbedding(ContextualEmbedding):
    ...
    def __init__(self, vocab: Vocabulary, model_dir_or_name: str = 'en-base-uncased', layers: str = '-1',
        ...
        self.model = _WordBertModel(model_dir_or_name=model_dir_or_name, vocab=vocab, layers=layers,
                                    ...
                                    only_use_pretrain_bpe=only_use_pretrain_bpe, **kwargs)
    ...
    def forward(self, words, classifier_func=None):
        r"""
        计算words的bert embedding表示。计算之前会在每句话的开始增加[CLS]在结束增加[SEP], 并根据include_cls_sep判断要不要
            删除这两个token的表示。

        :param torch.LongTensor words: [batch_size, max_len]
        :param torch.nn.Module classifier_func:
        ...
        outputs = self.model(words, classifier_func=classifier_func)
        loss = None
        ...
        if loss is not None:
            return self.dropout(outputs), loss
        return self.dropout(outputs)
    ...
    def predict(self, words):
        ...
        if outputs is not None:
            return self.dropout(outputs)
        outputs = self.model.predict(words)
        outputs = torch.cat([*outputs], dim=-1)
...
class _WordBertModel(nn.Module):
    def __init__(self, model_dir_or_name: str, vocab: Vocabulary, layers: str = '-1', pool_method: str = 'first',
                 ...
                 only_use_pretrain_bpe=False, **kwargs):
        super().__init__()

        self.tokenzier = BertTokenizer.from_pretrained(model_dir_or_name)

        if 'encoder_type' in kwargs:
            self.encoder = BertModel.from_pretrained(model_dir_or_name, encoder_type=kwargs['encoder_type'])
        else:
    ...
    def forward(self, words, classifier_func=None):
        ...
        input_args = {'input_ids': word_pieces,
                      'token_type_ids': token_type_ids,
                      'attention_mask': attn_masks,
                      'output_all_encoded_layers': True,
                      'classifier_func': classifier_func}
        input_args = _build_args(self.encoder.forward, **input_args)
        encoder_result = self.encoder(**input_args)
        if len(encoder_result) == 2:
            bert_outputs, pooled_cls = encoder_result
            loss = None
        elif len(encoder_result) == 3:
            bert_outputs, pooled_cls, loss = encoder_result
        else:
            raise RuntimeError
    ...
    def predict(self, words):
        ...
        # 2. 获取hidden的结果，根据word_pieces进行对应的pool计算
        # all_outputs: [batch_size x max_len x hidden_size, batch_size x max_len x hidden_size, ...]
        bert_outputs, pooled_cls = self.encoder.predict(word_pieces, token_type_ids=token_type_ids, attention_mask=attn_masks,
                                                        output_all_encoded_layers=True)
        # output_layers = [self.layers]  # len(self.layers) x batch_size x real_word_piece_length x hidden_size
